...
Uses the project's LLMProviderFactory and settings so the same
GENERATION_BACKEND, GENERATION_MODEL_ID, OPENAI_API_KEY, OPENAI_API_URL,
and COHERE_API_KEY from docker/env/.env.app drive generation.
...
async def fetch_chunks(
    ...
    return [
        Chunk(
            ...
            metadata=_safe_parse_metadata(row["chunk_metadata"]),
        )
        for row in rows
    ]
...
def fetch_chunks_from_qdrant_sync(
    ...
    for point in points:
        ...
        metadata = _safe_parse_metadata(payload.get("metadata"))
...
""".strip(),
    ...
    "ar": """
أنشئ {questions_per_chunk} مثال(أمثلة) تقييم مرشحة من جزء المستند أدناه.

القواعد:
- يجب أن يكون السؤال قابلاً للإجابة باستخدام هذا الجزء فقط.
- يجب أن تكون الإجابة المرجعية موجزة ومدعومة بالكامل بهذا الجزء.
- لا تخترع أسماء أو تواريخ أو سياسات أو أرقاماً أو وقائع.
- تجنب الأسئلة العامة مثل "ما هو هذا المستند؟".
- اكتب السؤال والإجابة باللغة العربية.
- أعد JSON فقط، كمصفوفة من الكائنات بالمفاتيح: query, reference_answer, difficulty.
- يجب أن تكون difficulty واحدة من: easy, medium, hard.

معرف الجزء: {chunk_id}
بيانات الجزء الوصفية: {chunk_metadata}
نص الجزء:
{chunk_text}
""".strip(),
}
...
def build_prompt(chunk: Chunk, questions_per_chunk: int, locale: str) -> str:
    template = PROMPT_TEMPLATES.get(locale, PROMPT_TEMPLATES["en"])
...
def create_generation_client(settings) -> Any:
...
async def generate_examples(
    ...
    prompt = build_prompt(chunk, questions_per_chunk, locale)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if not raw:
                raise RuntimeError("Generation client returned empty response")
            ...
            print(f"[RETRY] chunk_id={chunk.id}: rate limit hit, waiting {delay:.0f}s (attempt {attempt}/{max_retries})")
...
def make_candidate_row(
    ...
    metadata = {
        ...
        "reviewed": False,
        "generator_backend": settings.GENERATION_BACKEND,
        "generator_model": settings.GENERATION_MODEL_ID,
...
async def run(args: argparse.Namespace) -> None:
    ...
    with output_path.open("w", encoding="utf-8") as output_file:
        for chunk in chunks:
            ...
            if args.delay_between_chunks > 0:
                await asyncio.sleep(args.delay_between_chunks)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--retry-delay", type=float, default=2.0, help="Initial delay in seconds before retry")
    parser.add_argument("--delay-between-chunks", type=float, default=0.0, help="Pause between chunks to avoid rate limits")
