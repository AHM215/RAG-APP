...
from ..enums import OpenAIEnums, SystemPromptEnum
...
class OpenAiProvider(LLMInterface):
    ...
    def generate_text(self, prompt: str, chat_history: list = [], temperature: float = None, 
        ...
        if not chat_history:
            chat_history = [self.construct_prompt(self.default_system_prompt, role=OpenAIEnums.SYSTEM.value)]
    ...
    def construct_prompt(self, query: str, role: str = None, chat_history: list = []):
        if role:
            ...
            chat_history.append({"role": OpenAIEnums.USER.value, "content": self.process_text(query)})
        return chat_history
